<section class="relative w-full h-[300px] md:h-screen">
  ...
  <div class="absolute inset-x-0 bottom-5 flex justify-center space-x-3">
    @foreach ($slide as $index => $d)
    <button onclick="showSlide({{ $index }})" class="bg-white w-4 h-4 rounded-full slide-button"></button>
    @endforeach
  </div>
</section>
...
<script>
  ...
  function showSlide(slideIndex) {
    ...
    buttons.forEach((btn, index) => {
      btn.classList.toggle('bg-green-500', index === slideIndex); // Memberikan warna aktif
    });
